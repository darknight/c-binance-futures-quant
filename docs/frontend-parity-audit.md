...
Goal: decide when `react-front/` can be removed after the dashboard rewrite to `web-front/`.

## Summary

`web-front/` now covers the functional surface that is reachable from the current legacy frontend entrypoint:

- `react-front/src/App.js`
- `react-front/src/work/constainers/MySwitch.js`
- `react-front/src/work/constainers/Show.js`

The current legacy router maps only `/` to `Show`. Other files under `react-front/src/work/components/`, `reducers/`, `tradingview_extra_js/`, and related constants are historical or currently unreachable from the active route.

Do not delete `react-front/` until the "Needs owner decision" items below are explicitly accepted as deprecated or migrated.

## Covered Active Dashboard Features

| Legacy feature | Legacy source | New implementation | Status |
| --- | --- | --- | --- |
| KPI cards: total balance, position value, 24h commission, total profit, system status | `Show.js` `getQuantData()` and render cards | `web-front/src/components/KpiCards.tsx`, `/get_dashboard_summary`, `/get_profit_by_symbol` | Covered |
...
| Backend-driven dashboard data instead of CDN JSON | `Show.js` reads `CDN_BASE_URL` JSON | `web-front/src/api/dashboard.ts` calls FastAPI | Covered by design |

## Not Actively Routed In Legacy Frontend

These files are present under `react-front/`, but the active route only renders `Show`. Static references show these modules are not part of the current running legacy dashboard.
...
To claim "100% of `react-front/` directory capability" rather than "100% of active dashboard route", decide the following:

1. API key onboarding page:
   - Migrate to `web-front`, or confirm it is obsolete because keys now live in `.env` as `BINANCE_API_ARR`.

2. Hotkey and trading UI configuration:
   - Migrate to `web-front`, or confirm it is obsolete because `web-front` is a dashboard only.

3. Runtime `state_config_obj` editor:
   - Migrate to `web-front`, or keep configuration edited via `user_config.json`/backend API without UI.

4. TradingView charting library:
   - Migrate to `web-front`, or confirm it is not part of the dashboard replacement scope.
...
6. Legacy `CDN_BASE_URL` dashboard mode:
   - Confirm deletion because `web-front` intentionally reads FastAPI APIs instead of generated CDN JSON.

## Recommended Deletion Criteria

Delete `react-front/` only after all of these are true:

- `web-front` production build passes.
- Backend tests pass.
...
- Main docs no longer advertise `react-front/` as runnable.
