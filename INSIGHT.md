...
目标：不用真实 Binance 数据和真实下单，先证明内部交易闭环可以稳定、可重复地跑起来。

建议任务：

- 为 `ws-server` 增加一个 Python smoke producer/consumer：用真实 websocket 协议写入可控 tick/kline/position/balance，再读取 `B` 验证格式。
- 增加 dry-run runner：从真实 `ws-server` 快照读取数据，先用 deterministic demo rule 产生 order intent，但不调用 Binance。
- 第一刀把 dry-run order intent 作为 structured JSONL 输出到 stdout，让命令行、Docker logs 和测试能检查同一份输出。
- 第一刀不强制进入数据库、FastAPI 或 frontend dashboard。
- 把 dry-run intent 的数据库/API/dashboard 展示作为后续增强，而不是 Phase 2A 的首要验收。
- 不在 Phase 2A 首刀里重构完整的 `services/trading/simple_trade.py`，避免把内部闭环验证扩大成策略框架迁移。
- 第一验收场景只覆盖 `BTCUSDT` 空仓 + deterministic 1m 下跌，预期输出一个 `open_long` order intent；多 symbol、空头、平仓、止损止盈、余额不足等场景后置。

### Phase 2B：Binance Testnet Adapter

目标：在内部确定性闭环稳定后，再验证 Binance Futures testnet 这个外部交易所边界。

建议任务：

- 增加显式 testnet 配置，避免任何测试路径静默指向 production Binance。
- 用小而可控的场景验证 testnet 的行情、账户和订单 API。
- 即使 testnet adapter 可用，真实交易路径也继续默认关闭。
